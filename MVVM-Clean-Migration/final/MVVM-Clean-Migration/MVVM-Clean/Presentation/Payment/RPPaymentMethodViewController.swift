...
public class RPPaymentMethodViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        // start to load data
        self.viewModel.userAuthTrigger = ()
    }
...
extension RPPaymentMethodViewController {
    ...
    private func bindingUIs() {
        ...
        // data
        self.viewModel.$responseDataPublisher
            .dropFirst()
        ...
        // transit
        self.viewModel.$transitPublisher
            .dropFirst()
            .sink { [unowned self] entity in
                let storyboard = UIStoryboard(name: "Main", bundle: Bundle(for: Self.self))
                if let vc = storyboard.instantiateViewController(identifier: "RPCusomerDetailViewController") as? RPCusomerDetailViewController, let container = self.dependency as? PaymentFlowContainer {
